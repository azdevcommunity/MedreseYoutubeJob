...
public class YouTubeService : IYouTubeService
{
    ...
    public async Task<Core.Entities.Video?> GetVideoFromDbAsync(string videoId)
    {
    ...
    private async Task<List<Core.Entities.Playlist>> GetPlaylistsAsync()
    {
        var playlists = new List<Core.Entities.Playlist>();
            ...
            do
            {
                ...
                if (response.Items.Any())
                {
                    ...
                    var playlistsInPage = response.Items
                        ...
                        .Select(item => new Core.Entities.Playlist
                        {
                            PlaylistId = item.Id,
    ...
    private async Task<List<Core.Entities.Video>> GetVideosInPlaylistAsync(string playlistId)
    {
        var videos = new List<Core.Entities.Video>();
        string? nextPageToken = null;
        int pageCount = 0;
            ...
            do
            {
                ...
                if (response.Items.Any())
                {
                    ...
                    var videosInPage = response.Items
                        ...
                        .Select(item => new Core.Entities.Video
                        {
    ...
    private async Task SavePlaylistAndVideosAsync(Core.Entities.Playlist playlist, List<Core.Entities.Video> videos)
    {
    ...
    private async Task<(List<Core.Entities.Video>, List<PlaylistVideo>)> SearchVideosAsync(int maxResults)
    {
        var videos = new List<Core.Entities.Video>();
        ...
        do
        {
            ...
            if (response.Items.Any())
            {
                ...
                {
                    var newVideo = new Core.Entities.Video
                    {
